...
const TicketPrice = ({ ticket, setTicketsSelectedWithPrice, index, arabicTicketNames, locale, country }: Props) => {
    ...
    return (
        <div key={ticket.id} className='flex items-center justify-center flex-col gap-2 w-full mb-4'>
            ...
            <div className='flex pl-6 pr-12 items-center justify-between w-full'>
                ...
                <input value={ticket.salePrice} onChange={(e) => setTicketsSelectedWithPrice(prev => /^-?\d+(\.\d+)?$/.test(e.target.value) ? prev.map(ticketDoc => ticketDoc.id === ticket.id ? ({ ...ticket, salePrice: e.target.value }) as TicketType & { salePrice: string } : ticketDoc as TicketType & { salePrice: string }) : prev)} placeholder={`0.00 ${country}`} className='bg-[#1E1E1E] rounded-sm w-20 text-center h-7 text-white font-poppins text-sm font-normal' />
            </div>
        </div>
    )
}
